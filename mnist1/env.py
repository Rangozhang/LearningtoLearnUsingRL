...
class SoftmaxRegression(object):
  ...
  def step(self, action):
    ...
    post_c, post_c_batch, post_var = self.session.run([self.cost, self.cost_batch, self.trainable_var], \
            feed_dict={self.x:train_x, self.y:train_y, self.adaptive_lr_holder:self.adaptive_lr})
    ...
    log_diff = np.log(c) - np.log(post_c) if self.n_step != 0 else 0
...
if __name__ == "__main__":
  ...
  writer = tf.summary.FileWriter("./res/fig/{:10d}".format(int(time.time())), lr.session.graph)
