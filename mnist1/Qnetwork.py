...
# uncertain
tf.app.flags.DEFINE_integer('explore', 60000, 'observe before training')
tf.app.flags.DEFINE_integer('observe', 30000, 'observe before training') # as long as > batch_size
tf.app.flags.DEFINE_float('tau', 1e-3, 'rate to update target network towards primary network')
tf.app.flags.DEFINE_integer('memory_size', 30000, 'replay memory size')
...
class Qnetwork(object):
  ...
  def set_perception(self, action, reward, next_observ, terminal):
    ...
    state_str = 'state=' + state + ' e={:.5f}'.format(self.epsilon)
